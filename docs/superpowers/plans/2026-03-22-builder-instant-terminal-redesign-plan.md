...
export async function POST(req: Request) {
  try {
    ...
    // 1. Check Quota
    const hasQuota = await checkQuota(supabase, user.id, 'builder');
    if (!hasQuota) {
      return NextResponse.json({
        error: 'Builder quota exceeded. Please upgrade your plan or try again tomorrow.',
        code: 'QUOTA_EXCEEDED'
      }, { status: 429 });
    }

    // 2. Consume Quota
    await incrementUsage(supabase, user.id, 'builder');

    // 3. Create initial project record with 'analyzing' status
    // Basic idempotency could be added here if needed
    ...
    if (!bridgeResponse.ok) {
      // If bridge fails, fail open and proceed to build
...
git commit -m "feat(api): add builder analysis proxy route"
```
...
- [ ] **Step 1: Modify existing /api/builder/start/route.ts to accept answers**

```typescript
// Replace or modify src/app/api/builder/start/route.ts
import { NextResponse } from 'next/server';
import { createServerClient } from '@/lib/supabase/server';
import { BRIDGE_URL } from '@/config/platform';
import { checkQuota, incrementUsage } from '@/lib/quota';
...
export async function POST(req: Request) {
  try {
    ...
    // 1. Get project and verify ownership/status
    ...
    // If answers provided (from questionnaire)
    let fullDescription = project.description;
    if (answers) {
      // Check if project is older than 5 mins (meaning it's a retry/resume)
      // If it's fresh, quota was already consumed in /analyze
      const createdTime = new Date(project.created_at).getTime();
      const now = new Date().getTime();
      if (now - createdTime > 5 * 60 * 1000) {
         // It's a late resume, check and consume quota
         const hasQuota = await checkQuota(supabase, user.id, 'builder');
         if (!hasQuota) {
           return NextResponse.json({ error: 'Builder quota exceeded.', code: 'QUOTA_EXCEEDED' }, { status: 429 });
         }
         await incrementUsage(supabase, user.id, 'builder');
      }
    ...
    // 3. Trigger Bridge Build
    // We send fullDescription so Bridge gets everything
    const bridgeResponse = await fetch(`${BRIDGE_URL}/build/start`, {
      ...
      body: JSON.stringify({
        projectId,
        planText: fullDescription, // Bridge expects planText, we pass the full description
        userId: user.id
      })
    });
...
git commit -m "refactor(api): update start route to handle questionnaire answers"
```

### Task 5: AI Bridge - Chat Route
...
git commit -m "feat(bridge): add post-build chat proxy route"
```

### Task 6: Next.js API - Chat Route

**Files:**
- Create: `src/app/api/builder/chat/route.ts` (Actually, should be `src/app/api/builder/chat/[id]/route.ts`)
...
### Task 7: Questionnaire Form Component
...
### Task 9: Terminal Chat Component
...
export default function TerminalChat({ projectId, projectName, initialStatus }: TerminalChatProps) {
  ...
  useEffect(() => {
    if (initialStatus === 'building' || initialStatus === 'analyzing') {
        ...
        evtSource.onerror = () => {
            evtSource.close();
            // Don't mark as failed immediately on error, it might be a transient connection issue.
            // In a real app we'd have reconnection logic.
        };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
          ...
          if (line.startsWith('data: ')) {
            try {
              ...
              if (data.type === 'content') {
                 // Remove trailing newlines from tool output for cleaner terminal display
                 ...
                 // The backend has saved the message to DB and updated timestamps
      ...
      // Optionally refresh the iframe if it exists on the page
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)] bg-[#1a1a19] rounded-xl border border-[#333] overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 bg-[#222220] border-b border-[#333]">
        <div className="flex items-center gap-3">
          ...
          <span className="font-mono text-sm text-gray-300 ml-2">{projectName}</span>
...
git commit -m "feat(ui): add TerminalChat component"
```

### Task 10: Terminal Page
...
### Task 11: Modify Builder Form Submission
...
// Inside BuilderPage component (around line 30, replacing onSubmit logic):
...
```
*Note: The snippet above assumes you find the existing `onSubmit` in `src/app/(platform)/app/builder/page.tsx`. Apply the fetch change correctly to match existing state variables (`name`, `description`, `stack`, `setIsSubmitting`).*
